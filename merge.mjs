...
export function mergeProcessor (alpha = {}, omega = {}) {
  if (hasValidProcessor(omega)) {
    const processor = getProcessor(omega)

    if (typeof processor === 'string') return processor
    return { ...processor }
  }

  if (hasValidProcessor(alpha)) {
    const processor = getProcessor(alpha)
...
function hasValidProcessor (configuration) {
  const processor = getProcessor(configuration)

  if (isString(processor)) {
    if (hasPlugins(configuration)) {
      return (
        Reflect.has(getPlugins(configuration), processor)
      )
    }
  }
  ...
  if (isObject(processor)) {
    if (
      Reflect.has(processor, 'preprocess') &&
      Reflect.has(processor, 'postprocess')
    ) {
      return (
        Reflect.get(processor, 'preprocess') instanceof Function &&
        Reflect.get(processor, 'postprocess') instanceof Function
      )
    }
  }
...
export function getMergeWith (omega) {
  return function mergeWith (alpha) {
    return {
      ...
      ...(
        hasValidProcessor(alpha) || hasValidProcessor(omega)
